...
- [ ] **MCP-INJECT-01**: Xây dựng `open-sse/mcp/injector.js` chuyển đổi danh sách MCP Tools sang định dạng chuẩn (`mcp__<server>__<tool>`) cho OpenAI format (`tools: [{type: "function", function: ...}]`).
- [ ] **MCP-INJECT-02**: Hỗ trợ chuyển đổi Tool Schema sang định dạng Anthropic Claude (`tools: [{name, description, input_schema}]`).
- [ ] **MCP-INJECT-03**: Hỗ trợ chuyển đổi Tool Schema sang định dạng Google Gemini / Antigravity (`tools: [{functionDeclarations: [...]}]`).
